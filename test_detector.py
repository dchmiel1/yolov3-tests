...
class Test:
    ...
    def run(self):
        for img_file, real_objects in zip(self.img_files, self.real):
            std_out = subprocess.Popen(["./darknet", "detector", "test", data_file, cfg_file, weights_file, f"{self.path_to_test_files}/{img_file}", "-dont_show"], stdout=subprocess.PIPE).communicate()[0]
